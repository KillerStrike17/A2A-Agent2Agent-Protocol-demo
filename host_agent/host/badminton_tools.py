...
def list_court_availabilities(date: str) -> dict:
    """
    Lists the available and booked time slots for a badminton court on a given date.
...
def book_badminton_court(
    ...
    """
    Books a badminton court for a given date and time range under a reservation name.
    ...
    return {
        "status": "success",
        "message": f"Success! The badminton court has been booked for {reservation_name} from {start_time} to {end_time} on {date}.",
    }